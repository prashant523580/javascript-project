    ...
    fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${search_meal}`)
        ...
        .then(item => {
            ...
            if (item.meals) {
                ...
                <div class="item-name">
                    <p>${ele.strMeal}</p>
                    <a href="#recipe-details${index}" class="recipe-btn">Details</a>
                </div>
            </div>
    ...
    if (e.target.classList.contains("recipe-btn")) {
        ...
        fetch(`https://www.themealdb.com/api/json/v1/1/lookup.php?i=${item.dataset.id}`)
            ...
            .then(data => {
                ...
                <p class="recipe-category"> Area : ${ele.strArea}</p>
                <div class="instruct">
                    <h1>instructions</h1>
                ...
                <div class="recipe-link">
                    <a href="${ele.strYoutube}" target="_blank">watch video</a>
                </div>
...
let search_recipe  =document.getElementById("search");

search_recipe.addEventListener("keyup",() =>{
    let submit_btn = document.getElementById("submit-btn");
    if(search_recipe.value.trim().length > 0){

        submit_btn.innerHTML = "get recipe";
    }else{
        submit_btn.innerHTML = "get all data";
    }
} )
